...
if ( ! function_exists( 'loading_page_admin_resources' ) ) {
	...
	function loading_page_admin_resources( $hook ) {
		if ( strpos( $hook, 'loading-page' ) !== false ) {
			...
			wp_enqueue_script( 'lp-admin-script', LOADING_PAGE_PLUGIN_URL . '/js/loading-page-admin.js', array( 'jquery', 'thickbox', 'farbtastic' ), 'free-1.0.87', true );
		}
...
if ( ! function_exists( 'loading_page_enqueue_scripts' ) ) {
	...
	function loading_page_enqueue_scripts() {
		...
		if ( $loading_screen && ! empty( $op['from_trigger'] ) ) {
			wp_enqueue_script( 'codepeople-loading-page-link-script', LOADING_PAGE_PLUGIN_URL . '/js/links.min.js', array( 'jquery' ), 'free-1.0.87', false );
		}
		...
		if ( $loading_screen ) {
			...
			wp_enqueue_style( 'codepeople-loading-page-style', LOADING_PAGE_PLUGIN_URL . '/css/loading-page.css', array(), 'free-1.0.87', false );
			wp_enqueue_style( 'codepeople-loading-page-style-effect', LOADING_PAGE_PLUGIN_URL . '/css/loading-page' . ( ( 'none' !== $op['pageEffect'] ) ? '-' . $op['pageEffect'] : '' ) . '.css', array(), 'free-1.0.87', false );

			$s = loading_page_get_screen( $op['loading_screen'] );
			if ( $s ) {
				if ( ! empty( $s['style'] ) ) {
					wp_enqueue_style( 'codepeople-loading-page-style-' . $s['id'], $s['style'], array(), 'free-1.0.87', false );
				}

				if ( ! empty( $s['script'] ) ) {
					wp_enqueue_script( 'codepeople-loading-page-script-' . $s['id'], $s['script'], array( 'jquery' ), 'free-1.0.87', false );
					$required[] = 'codepeople-loading-page-script-' . $s['id'];
				}
			}
			wp_enqueue_script( 'codepeople-loading-page-script', LOADING_PAGE_PLUGIN_URL . '/js/loading-page.min.js', $required, 'free-1.0.87', false );
			...
			<form method="post">
				...
				<div class="postbox">
					...
					<div class="inside lp-video-tutorial <?php echo esc_attr( $loading_page_video_tutorial_status ); ?>" style="position:relative;">
						...
						<div class="lp-video-container">
							<iframe title="<?php print esc_attr( __( 'Video tutorial', 'loading-page' ) ); ?>" src="https://www.youtube.com/embed/5x_LtjoCFUY" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
						</div>
					</div>
				</div>
				<div class="postbox">
					...
					<div class="inside">
						...
						<table class="form-table">
							<tr>
								...
								<td><input aria-label="<?php print esc_attr( __( 'Enable loading screen', 'loading-page' ) ); ?>" type="checkbox" name="lp_enabled_loading_screen" <?php echo ( ( ! empty( $loading_page_options['enabled_loading_screen'] ) ) ? 'CHECKED' : '' ); ?> /></td>
							</tr>
							<tr>
								...
								<td style="border-top:2px dashed green;border-right:2px dashed green;border-bottom:2px dashed green;padding-left:10px;"><input aria-label="<?php print esc_attr( __( 'Show loading screen when clicking on link', 'loading-page' ) ); ?>" type="checkbox" name="lp_from_trigger" <?php print ! empty( $loading_page_options['from_trigger'] ) ? 'CHECKED' : ''; ?> /> <i style="color:green;"><?php esc_html_e( 'Display the loading screen as soon as the link on the current page is clicked, and not only when the next page is loaded.', 'loading-page' ); ?></i></td>
							</tr>
							<tr>
								<th><?php esc_html_e( 'Display a close screen button', 'loading-page' ); ?></th>
								<td><input aria-label="<?php print esc_attr( __( 'Display a close screen button', 'loading-page' ) ); ?>" type="checkbox" name="lp_close_btn" <?php echo ( ( ! isset( $loading_page_options['close_btn'] ) || $loading_page_options['close_btn'] ) ? 'CHECKED' : '' ); ?> /></td>
							</tr>
							<tr>
							...
							<tr>
								...
								<td>
									<select aria-label="<?php print esc_attr( __( 'Screen size', 'loading-page' ) ); ?>" name="lp_screen_size" style="float:left;">
										<option value="all"
										<?php
										if (
									...
									?>
										...
										<input aria-label="<?php print esc_attr( __( 'Screen width', 'loading-page' ) ); ?>" type="text" name="lp_screen_width" value="
																		<?php
							...
							<tr>
								...
								<td>
									...
									?>
									<input aria-label="<?php print esc_attr( __( 'Always', 'loading-page' ) ); ?>" type="radio" value="always" name="once_per_session" <?php echo ( ( 'always' === $loading_page_options['once_per_session'] ) ? 'CHECKED' : '' ); ?> /> <span><?php esc_html_e( 'always', 'loading-page' ); ?></span><br />
									<input aria-label="<?php print esc_attr( __( 'Once per session', 'loading-page' ) ); ?>" type="radio" value="site" name="once_per_session" <?php echo ( ( 'site' === $loading_page_options['once_per_session'] ) ? 'CHECKED' : '' ); ?> /> <span><?php esc_html_e( 'once per session', 'loading-page' ); ?></span><br />
									<input aria-label="<?php print esc_attr( __( 'Once per page', 'loading-page' ) ); ?>" type="radio" value="page" name="once_per_session" <?php echo ( ( 'page' === $loading_page_options['once_per_session'] ) ? 'CHECKED' : '' ); ?> /> <span><?php esc_html_e( 'once per page', 'loading-page' ); ?></span>
							...
							<tr>
								...
								<td>
									<div><input aria-label="<?php print esc_attr( __( 'Homepage', 'loading-page' ) ); ?>" type="radio" name="lp_loading_screen_display_in" value="home" <?php echo ( ( isset( $loading_page_options['lp_loading_screen_display_in'] ) && 'home' === $loading_page_options['lp_loading_screen_display_in'] ) ? 'CHECKED' : '' ); ?> /> <?php esc_html_e( 'homepage only', 'loading-page' ); ?></div>

									<div><input aria-label="<?php print esc_attr( __( 'All pages', 'loading-page' ) ); ?>" type="radio" name="lp_loading_screen_display_in" value="all" <?php echo ( ( isset( $loading_page_options['lp_loading_screen_display_in'] ) && 'all' === $loading_page_options['lp_loading_screen_display_in'] ) ? 'CHECKED' : '' ); ?> /> <?php esc_html_e( 'all pages', 'loading-page' ); ?></div>

									<div><input aria-label="<?php print esc_attr( __( 'Specific pages', 'loading-page' ) ); ?>" type="radio" name="lp_loading_screen_display_in" value="pages" <?php echo ( ( isset( $loading_page_options['lp_loading_screen_display_in'] ) && 'pages' === $loading_page_options['lp_loading_screen_display_in'] ) ? 'CHECKED' : '' ); ?> /> <?php esc_html_e( 'specific pages', 'loading-page' ); ?>
										<input aria-label="<?php print esc_attr( __( 'Pages/posts ids', 'loading-page' ) ); ?>" type="text" name="lp_loading_screen_display_in_pages" value="
																		<?php
									...
									<div><input aria-label="<?php print esc_attr( __( 'specific post types', 'loading-page' ) ); ?>" type="radio" name="lp_loading_screen_display_in" value="post_type" <?php echo ( ( isset( $loading_page_options['lp_loading_screen_display_in'] ) && 'post_type' === $loading_page_options['lp_loading_screen_display_in'] ) ? 'CHECKED' : '' ); ?> /> <?php esc_html_e( 'specific post types', 'loading-page' ); ?>
										<input aria-label="<?php print esc_attr( __( 'Post types', 'loading-page' ) ); ?>" type="text" name="lp_loading_screen_display_post_types" value="
																		<?php
							...
							<tr>
								...
								<td><input aria-label="<?php print esc_attr( __( 'Pages/posts ids separated by comma', 'loading-page' ) ); ?>" type="text" name="lp_loading_screen_exclude_from_pages" value="
																	<?php
							...
							<tr>
								...
								<td><input aria-label="<?php print esc_attr( __( 'Post types separated by comma', 'loading-page' ) ); ?>" type="text" name="lp_loading_screen_exclude_from_post_types" value="
																	<?php
							...
							<tr>
								...
								<td><textarea aria-label="<?php print esc_attr( __( 'URLs of the pages to exclude', 'loading-page' ) ); ?>" name="lp_loading_screen_exclude_from_urls" style="width:100%" rows="6">
																		<?php
																																																				print esc_textarea(
							...
							<tr>
								<th colspan="2">
									...
									<input aria-label="<?php print esc_attr( __( 'Disable the loading screen from Coming Soon or Maintenance modes', 'loading-page' ) ); ?>" type="checkbox" name="lp_loading_screen_exclude_from_elementor_maintenance" <?php echo ( ( ! empty( $loading_page_options['lp_loading_screen_exclude_from_elementor_maintenance'] ) ) ? 'CHECKED' : '' ); ?> />
								</th>
							</tr>
							<tr>
								<td colspan="2">
							...
							<tr>
								...
								<td>
									<select aria-label="<?php print esc_attr( __( 'Loading screen', 'loading-page' ) ); ?>" name="lp_loading_screen">
										<?php
							...
							foreach ( $loading_screens as $screen ) {
								...
								if ( ! empty( $screen['adminscript'] ) ) {
									wp_enqueue_script( $screen['adminscript'], $screen['adminscript'], array(), 'free-1.0.87', true );
								}
							}
							?>
							<tr>
								...
								<td>
									<input aria-label="<?php print esc_attr( __( 'Background color', 'loading-page' ) ); ?>" type="text" name="lp_backgroundColor" id="lp_backgroundColor" value="
																	<?php
																	if ( isset( $loading_page_options['backgroundColor'] ) ) {
																		print( esc_attr( $loading_page_options['backgroundColor'] ) );}
																	?>
									" />
									<input aria-label="<?php print esc_attr( __( 'Apply transparency', 'loading-page' ) ); ?>" type="checkbox" name="lp_backgroundTransparency" <?php print ( ! isset( $loading_page_options['transparency'] ) || $loading_page_options['transparency'] ) ? 'CHECKED' : ''; ?> /><?php esc_html_e( 'Apply transparency', 'loading-page' ); ?>
							...
							<tr>
								...
								<td>
									<input aria-label="<?php print esc_attr( __( 'Background image', 'loading-page' ) ); ?>" type="text" name="lp_backgroundImage" id="lp_backgroundImage" value="
																	<?php
									...
									<select aria-label="<?php print esc_attr( __( 'Background position', 'loading-page' ) ); ?>" id="lp_backgroundRepeat" name="lp_backgroundRepeat">
										<option value="repeat"
										<?php
							...
							<tr>
								...
								<td>
									<input aria-label="<?php print esc_attr( __( 'Fullscreen', 'loading-page' ) ); ?>" type="checkbox" name="lp_fullscreen" id="lp_fullscreen" <?php echo ( ( isset( $loading_page_options['fullscreen'] ) && $loading_page_options['fullscreen'] ) ? 'CHECKED' : '' ); ?> />
							...
							<tr>
								...
								<td><input aria-label="<?php print esc_attr( __( 'Foreground color', 'loading-page' ) ); ?>" type="text" name="lp_foregroundColor" id="lp_foregroundColor" value="
																	<?php
							...
							<tr>
								...
								<td>
									<input aria-label="<?php print esc_attr( __( 'Additional seconds', 'loading-page' ) ); ?>" type="text" name="lp_additionalSeconds" id="lp_additionalSeconds" value="
																	<?php
							...
							<tr>
								...
								<td>
									<textarea aria-label="<?php print esc_attr( __( 'Ad or block of code', 'loading-page' ) ); ?>" name="lp_codeBlock" id="lp_codeBlock" rows="6" style="width:80%;">
																		<?php
							...
							<tr>
								...
								<td>
									<select aria-label="<?php print esc_attr( __( 'Animation effect', 'loading-page' ) ); ?>" name="lp_pageEffect">
										<?php
							...
							<tr>
								...
								<td><input aria-label="<?php print esc_attr( __( 'Display loading percent', 'loading-page' ) ); ?>" type="checkbox" name="lp_displayPercent" <?php echo ( ( ! empty( $loading_page_options['displayPercent'] ) ) ? 'CHECKED' : '' ); ?> /></td>
							</tr>
						</table>
						<div style="border: 1px solid #DADADA; padding:10px;">
							...
							<table class="form-table">
								<tr>
									...
									<td>
										<input aria-label="<?php print esc_attr( __( 'Disable search in deep', 'loading-page' ) ); ?>" type="checkbox" name="lp_deactivateDeepSearch" <?php echo ( ( empty( $loading_page_options['deepSearch'] ) ) ? 'CHECKED' : '' ); ?> /> <i><?php esc_html_e( 'If the loading screen stops in some percentage, tick the checkbox', 'loading-page' ); ?></i>
									</td>
								</tr>
								<tr>
									<th><?php esc_html_e( 'The pages\' background is visible', 'loading-page' ); ?></th>
									<td>
										<input aria-label="<?php print esc_attr( __( 'Tick if page visible', 'loading-page' ) ); ?>" type="checkbox" name="lp_modifyDisplayRule" <?php echo ( ( ! empty( $loading_page_options['modifyDisplayRule'] ) ) ? 'CHECKED' : '' ); ?> /> <i><?php esc_html_e( 'If website\'s background is visible before the loading screen, tick the checkbox', 'loading-page' ); ?></i>
									</td>
								</tr>
				...
				<div class="postbox">
					...
					<div class="inside">
						...
						<p><img alt="<?php print esc_attr( __( 'Lazy loading chart', 'loading-page' ) ); ?>" src="<?php print( esc_attr( LOADING_PAGE_PLUGIN_URL . '/images/consumption_graph.png' ) ); ?>" style="max-width:100%;" /></p>
						<table class="form-table">
							<tr>
								<th><?php esc_html_e( 'Enable lazy loading', 'loading-page' ); ?></th>
								<td><input aria-label="<?php print esc_attr( __( 'Enable lazy loading', 'loading-page' ) ); ?>" type="checkbox" DISABLED /></td>
							</tr>
							<tr>
								<th><?php esc_html_e( 'Select the image to load by default', 'loading-page' ); ?></th>
								<td>
									<input aria-label="<?php print esc_attr( __( 'Image to load by default', 'loading-page' ) ); ?>" type="text" DISABLED /><input type="button" value="Browse" DISABLED />
								</td>
							</tr>
							<tr>
								<th><?php esc_html_e( 'Exclude lazy loading from', 'loading-page' ); ?></th>
								<td><input aria-label="<?php print esc_attr( __( 'Pages/posts ids separated by comma', 'loading-page' ) ); ?>" type="text" DISABLED /> <i><?php esc_html_e( 'Type one, or more post/pages IDs, separated by commas ","', 'loading-page' ); ?></i></td>
							</tr>
						</table>
						<div style="border: 1px solid #DADADA; padding:10px;">
							...
							<table class="form-table">
								<tr>
									...
									<td>
										<input aria-label="<?php print esc_attr( __( 'Class or attributes names', 'loading-page' ) ); ?>" type="text" style="width:100%;" disabled /><br />
