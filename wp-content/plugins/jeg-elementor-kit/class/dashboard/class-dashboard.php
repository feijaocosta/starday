...
class Dashboard {
	...
	public function init_dashboard() {
		...
		if ( $pagenow == 'admin.php' && isset( $_GET['page'] ) && self::$dashboard === $_GET['page'] ) {
			wp_safe_redirect( menu_page_url( self::$settings, false ) );
			exit;
		}

		if ( $pagenow == 'admin.php' && isset( $_GET['page'] ) && self::$templates === $_GET['page'] ) {
	...
	public function render_header_dashboard() {
		if ( is_admin() && isset( $_REQUEST['page'] ) && strpos( $_REQUEST['page'], 'jkit' ) !== false ) {
			...
			<div class="jkit-dashboard-topbar-wrap">
				<img src="<?php echo JEG_ELEMENTOR_KIT_URL . '/assets/img/admin/icon-dashboard.svg'; ?>" alt="<?php echo JEG_ELEMENTOR_KIT_NAME; ?>">
				<h2><?php echo JEG_ELEMENTOR_KIT_NAME; ?></h2>
				<ul>
					<li><a href="https://jegtheme.com/" target="_blank"><i class="fa fa-globe"></i></a></li>
					<li><a href="https://wordpress.org/support/plugin/jeg-elementor-kit/" target="_blank"><i class="fa fa-support"></i></a></li>
					<li><a href="https://www.instagram.com/jegtheme/" target="_blank"><i class="fa fa-instagram"></i></a></li>
				</ul>
			</div>
			<?php
		}
