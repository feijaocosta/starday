				...
				switch ( $option['type'] ) {
						...
						if ( $repeater_options ) {
							...
							if ( isset( $option['responsive'] ) && $option['responsive'] ) {
								...
								foreach ( jkit_get_responsive_breakpoints() as $breakpoint ) {
									$args[ $breakpoint['key'] . '_default' ] = isset( $option[ $breakpoint['key'] . '_default' ] ) ? $option[ $breakpoint['key'] . '_default' ] : $default_value;
								}
					...
					case 'radio':
						$args = array(
							...
							'default'   => isset( $option['default'] ) ? $option['default'] : '',
							'toggle'    => isset( $option['toggle'] ) ? $option['toggle'] : '',
							'separator' => isset( $option['separator'] ) ? $option['separator'] : '',
						);
