<tr id="loading_screen_logo" style="display:none;" class="lp_ls_section">
	...
	?>
	<input aria-label="<?php print esc_attr(__('Logo image', LOADING_PAGE_TD)); ?>" type='text' name="lp_ls[logo][image]" id="lp_ls_logo_image" value="<?php
		print esc_attr($loading_page_logo_path);
	?>" /><input type="button" value="Browse" onclick="loading_page_selected_image('lp_ls[logo][image]');" />&nbsp;&nbsp;&nbsp;
    <input type="checkbox" id="lp_ls_logo_grayscale" <?php if($loading_page_logo_grayscale) print 'CHECKED';?>>
    <input type="hidden" name="lp_ls[logo][grayscale]" value="<?php print esc_attr($loading_page_logo_grayscale); ?>">
    <?php
        _e('Grayscale to color effect', LOADING_PAGE_TD);
    ?>
    &nbsp;&nbsp;-&nbsp;&nbsp;
    <input type="checkbox" id="lp_ls_logo_blink" <?php if($loading_page_logo_blink) print 'CHECKED';?>>
    <input type="hidden" name="lp_ls[logo][blink]" value="<?php print esc_attr($loading_page_logo_blink); ?>">
    <?php
        _e('Apply blink effect', LOADING_PAGE_TD);
    ?>
    <br>
	<style>
 ...
 <?php
	...
	foreach($files as $file)
	{
		...
		{
			print '<object data="'.esc_attr($path).'" type="image/svg+xml" width="100" height="100">'.$file.'</object>';
		}
		else
		{
